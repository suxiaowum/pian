<template>
  <div id="code">
    <headerType :type="3"></headerType>
    <div class="marpad show_down">
      <div class="strps">
        <el-steps :active="active" align-center>
          <el-step title="选择分组与设备"></el-step>
          <el-step title="扫码登陆"></el-step>
        </el-steps>
      </div>
      <div class="selectBox" style="text-align: center" v-if="active == 1">
        <div>
          <span style="font-size: 14px"> 选择分组： </span>
          <el-select
            v-model="groupId"
            placeholder="选择一个分组"
            size="small"
            @change="groupChange"
          >
            <el-option
              v-for="(item, index) in gropList"
              :key="index"
              :label="item.name"
              :value="item.id"
            >
            </el-option>
          </el-select>
          <span class="addGroup" @click="addGroup = true">
            <i class="el-icon-circle-plus"></i>
          </span>
        </div>
        <div>
          <span style="font-size: 14px"> 选择设备： </span>
          <el-select
            v-model="equipmentId"
            placeholder="选择一个分组"
            size="small"
            @change="groupChange"
          >
            <el-option
              v-for="(item, index) in equipmentList"
              :key="index"
              :label="item.name"
              :value="item.id"
            >
            </el-option>
          </el-select>
          <span class="addGroup" style="opacity: 0;" >
            <i class="el-icon-circle-plus"></i>
          </span>
        </div>
      </div>
      <div class="next">
        <el-button
          v-if="active < 3"
          style="margin-top: 12px"
          @click="next"
          size="mini"
          :disabled="overState"
          >下一步</el-button
        >
        <el-button
          v-else
          type="primary"
          style="margin-top: 12px"
          @click="over"
          size="mini"
          >完成</el-button
        >
      </div>
    </div>
    <el-dialog
      title="新增分组"
      :visible.sync="addGroup"
      width="30%"
      :before-close="addGroupClose"
    >
      <el-form
        :model="form"
        ref="form"
        :rules="rules"
        label-width="80px"
        :inline="false"
        size="mini"
      >
        <el-form-item label="分组名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入分组名称"></el-input>
        </el-form-item>
      </el-form>

      <span slot="footer" class="dialog-footer">
        <el-button @click="addGroup = false">取 消</el-button>
        <el-button type="primary" @click="addGroup = false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
<script>
import headerType from "@/components/header/index.vue";
export default {
  components: {
    headerType,
  },
  data() {
    return {
      equipmentId: "",
      equipmentList: [],
      active: 1,
      addGroup: false,
      overState: true,
      groupId: "",
      gropList: [],
      form: {
        name: "",
      },
      rules: {
        name: [{ required: true, message: "请输入活动名称", trigger: "blur" }],
      },
    };
  },
  methods: {
    addGroupClose() {},
    groupChange() {
      if (this.equipmentId != "" && this.groupId != "") {
        this.overState = false;
      }
    },
    over() {},
    next() {},
  },
};
</script>
<style lang="scss" scoped>
.selectBox {
  text-align: center;
  margin-top: 20px;
}
.addGroup {
  font-size: 32px;
  color: #409eff;
  position: relative;
  top: 5px;
  left: 10px;
}
.next {
  text-align: center;
}
</style>